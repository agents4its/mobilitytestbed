...
public class PlanItem implements Cloneable {
	...
	public PlanItem(Request request, boolean getIn) {
	 ...
	 *            this plan item represents one of two nodes of this request
	 * @param getIn
	 *            true, if it represents the start node, false, if it's the
	 *            target node
	 * @param arrivalTime
	 *            the time when the taxi arrives into this node
	...
	private PlanItem(Request request, boolean getIn, long arrivalTime) {
		this.request = request;
		this.getIn = getIn;
	 ...
	 * 
	 * @param timeToAdd
	 *            time to add to the current arrival time
	 ...
	 * 
	 * @param timeToTake
	 *            the amount of time we should subtract
	...
	public boolean hasDelay(long arrivalTime) {
		return PlanItem.hasDelay(request, getIn, arrivalTime);
	}
