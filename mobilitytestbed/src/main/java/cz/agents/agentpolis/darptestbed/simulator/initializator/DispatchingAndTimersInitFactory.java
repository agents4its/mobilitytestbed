 ...
 * @author Lukas Canda
 */
public class DispatchingAndTimersInitFactory implements InitModulFactory {
	...
	public AbstractModule injectModule(Injector injector) {
		...
		if (!GlobalParams.isCentralized()) {
			...
			DriverCentrMessageProtocol driverCentrMessageProtocol = injector
					.getInstance(DriverCentrMessageProtocol.class);
			...
			// 		break;
			default:
				// by default, load the DispatchingLogicExample class for dispatcher
				logic = new DispatchingLogicExample("Dispatching", sender, driverCentrMessageProtocol, taxiModel,
